...
import org.ensime.protocol.swank.SwankProtocolConversions
import org.ensime.protocol.{ AnalyzerReadyEvent, IndexerReadyEvent, ProtocolEvent }
...
object IntgUtil extends Assertions with SLF4JLogging {
  ...
  def withTestProject(path: String)(f: (EnsimeConfig, Project, AsyncMsgHelper) => Unit): Unit = {
    ...
    withTempDirectory { base =>
      ...
      assert(connInfo.protocolVersion == "0.8.9")

      val c = new SwankProtocolConversions()
      val configResult = project.rcpInitProject(c.toWF(config))

      assert(configResult == project.config)

      val asyncHelper = new AsyncMsgHelper(actorSystem)

      project.rpcSubscribeAsync(event => {
        asyncHelper.asyncReceived(event)
      }, replayPrevious = true)
