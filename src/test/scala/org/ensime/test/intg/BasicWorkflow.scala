  ...
  describe("Server") {
    it("should open the test project", SlowTest) {
      ...
      IntgUtil.withTestProject("src/example-simple") { (config, project, asyncHelper) =>
        ...
        val (intTypeId, _) = symbolAtPointOpt match {
          case Some(SymbolInfo("scala.Int", "Int", None, BasicTypeInfo("Int", typeId, 'class, "scala.Int", List(), List(), None, None), false, Some(ownerTypeId))) =>
            (typeId, ownerTypeId)
          case _ =>
            throw new IllegalStateException("Symbol at point does not match expectations")
        }

        val fooClassByNameOpt = project.rpcTypeByName("org.example.Foo")
        val fooClassId = fooClassByNameOpt match {
          case Some(BasicTypeInfo("Foo", fooTypeIdVal, 'class, "org.example.Foo", List(), List(), Some(LineSourcePosition(`fooFile`, 0)), None)) =>
            fooTypeIdVal
          case _ =>
            throw new IllegalStateException("type by name for Foo class does not match expectations")
        }
        ...
        val fooObjectId = fooObjectByNameOpt match {
          case Some(BasicTypeInfo("Foo$", fooObjectIdVal, 'object, "org.example.Foo$", List(), List(), Some(OffsetSourcePosition(`fooFile`, 28)), None)) =>
            fooObjectIdVal
          case _ =>
            throw new IllegalStateException("type by name for Foo object does not match expectations")
        }
        ...
        val intTypeInspectInfo = typeByIdOpt match {
          ...
          case _ =>
            throw new IllegalStateException("type by id does not match expectations")
        }
        ...
        inspectByIdOpt match {
          ...
          case _ =>
            throw new IllegalStateException("inspect by id does not match expectations")
        }
        ...
        useOfSymbolAtPoint match {
          ...
          case _ =>
            throw new IllegalStateException("rpcUsesOfSymAtPoint not match expectations")
        }
        ...
        testMethodSymbolInfo match {
          ...
          case _ =>
            throw new IllegalStateException("symbol at point (local test method)")
        }
        ...
        insPacByPathResOpt match {
          ...
          case _ =>
            throw new IllegalStateException("inspect package by path")
        }
        ...
        prepareRefactorRes match {
          ...
          case _ =>
            throw new IllegalStateException("Prepare refactor result does not match")
        }
        ...
        execRefactorRes match {
          ...
          case _ =>
            throw new IllegalStateException("exec refactor does not match expectation: " + execRefactorRes)
        }
        ...
        val undoId = peekUndoRes match {
          case Right(Undo(undoIdVal, "Refactoring of type: 'rename", List(TextEdit(`fooFile`, 214, 217, "foo"), TextEdit(`fooFile`, 252, 255, "foo"), TextEdit(`fooFile`, 269, 272, "foo")))) =>
            undoIdVal
          case _ =>
            throw new IllegalStateException("unexpected peek undo result: " + peekUndoRes)
        ...
        execUndoRes match {
          ...
          case _ =>
            throw new IllegalStateException("unexpected exec undo result: " + execUndoRes)
        }
        ...
        packageMemberCompRes match {
          case List(
            ...
            CompletionInfo("MapProxy$", CompletionSignature(List(), ""), -1, false, 50, None)) =>
          case _ =>
            throw new IllegalStateException("package name completion result: " + packageMemberCompRes)
        }
      }
    }
  }
